...
<layout:head>
	...
	<static:script fileName="biblivre.holding.search.js" />
	<static:script fileName="biblivre.circulation.lending.js" />
					...
					{#foreach $T.lendingInfo as info}
						<div class="result user_lending" rel="{$T.info.lending.id}">
							<div class="record">
								...
								<label><i18n:text key="circulation.lending.lending_date" /></label>: {__formatDate($T.info.lending.created,'f')}<br/>
								{#if $T.info.lending.expectedReturnDate}<label><i18n:text key="circulation.lending.expected_return_date" /></label>: {__formatDate($T.info.lending.expectedReturnDate, 'D')}<br/>{#/if}
								...
								{#if $T.info.lending.daysLate > 0}
									...
									<label><i18n:text key="circulation.lending.daily_fine" /></label>: <c:out value="${currencySymbol}" /> {_f($T.info.lending.dailyFine || 0, 'n2') }<br/>
									<label><i18n:text key="circulation.lending.estimated_fine" /></label>: <span class="value_error"><c:out value="${currencySymbol}" /> {_f($T.info.lending.estimatedFine || 0, 'n2') }</span><br/>
								{#/if}
							</div>
							...
							<div class="record">
								...
								{#if $T.record.lending !== undefined}
									<div class="user_lending">
										...
										<label><i18n:text key="circulation.lending.lending_date" /></label>: {__formatDate($T.record.lending.created,'f')}<br/>
										{#if $T.record.lending.expectedReturnDate}<label><i18n:text key="circulation.lending.expected_return_date" /></label>: {__formatDate($T.record.lending.expectedReturnDate, 'D')}<br/>{#/if}
										...
										{#if $T.record.lending.daysLate > 0}
											...
											<label><i18n:text key="circulation.lending.daily_fine" /></label>: <c:out value="${currencySymbol}" /> {_f($T.record.lending.dailyFine || 0, 'n2') }<br/>
											<label><i18n:text key="circulation.lending.estimated_fine" /></label>: <span class="value_error"><c:out value="${currencySymbol}" /> {_f($T.record.lending.estimatedFine || 0, 'n2') }</span><br/>
										{#/if}
									</div>
								{#/if}
	...
	<div id="fine_popup" class="popup">
		...
		<fieldset class="fine">
			...
			<div class="description">
				...
				<label><i18n:text key="circulation.lending.daily_fine" /></label>: <c:out value="${currencySymbol}" /> <span class="daily_fine"></span><br/>
				<label><i18n:text key="circulation.lending.fine_value" /></label>: <c:out value="${currencySymbol}" /> <input type="text" name="fine_value" /><br/>
			</div>
