										...
										<%@ taglib prefix="i18n" uri="/WEB-INF/tlds/translations.tld" %>
											...
											<% ConfigurationBO configurationBO=(ConfigurationBO)
												...
												<jsp:useBean id="isMultiPart" type="java.lang.Boolean"
													scope="request" />
												<jsp:useBean id="isDisableMenu" type="java.lang.Boolean"
													scope="request" />
												<jsp:useBean id="isBanner" type="java.lang.Boolean" scope="request" />
												<jsp:useBean id="isSchemaSelection" type="java.lang.Boolean"
													scope="request" />
												...
												<body>
													<form id="page_submit" name="page_submit" method="post"
														${isMultiPart ? pageScope.multiPartAttributes :
														pageScope.notMultiPartAttribute}>
														<input type="hidden" name="controller" id="controller"
															value='${isMultiPart ? "json" : "jsp"}'>
															...
															<div id="title">
																<div id="logo_biblivre_small">
																	<a href="http://biblivre.org.br/" target="_blank">
																		<img src="<%= contextPath %>/static/images/logo_biblivre_small.png"
																			width="43" height="36" alt="Biblivre V">
																	</a>
																</div>
																<h1><a href="?">
																		<%= configurationBO.getString(Constants.CONFIG_TITLE)
																			%>
																	</a></h1>
																<h2>
																	<%= configurationBO.getHtml(Constants.CONFIG_SUBTITLE)
																		%>
																</h2>
															</div>
															<c:if test="${languages.size() > 1}">
																<div id="language_selection">
																	<select class="combo combo_auto_size" name="i18n"
																		onchange="Core.submitForm('menu', 'i18n', 'jsp');">
																		<c:forEach items="${languages}" var="dto">
																			<c:set var="selectedAttr">
																				<% TranslationsMap
																					translations=(TranslationsMap)
																					request.getAttribute("translationsMap");
																					%>
																					${translationsMap.getLanguage().equals(dto.getLanguage())
																					? "selected" : ""}
																			</c:set>
																			<option value="${dto.getLanguage()}"
																				${selectedAttr}>${dto.toString()}
																			</option>
																		</c:forEach>
																	</select>
																</div>
																			...
																			<c:choose>
																				<c:when test="${isLogged}">
																					<menu:level module="multi_schema">
																						<menu:item module="multi_schema"
																							action="administration_password" />
																						<menu:item module="multi_schema"
																							action="multi_schema_manage" />
																						<menu:item module="multi_schema"
																							action="multi_schema_configurations" />
																						<menu:item module="multi_schema"
																							action="multi_schema_translations" />
																						<menu:item module="multi_schema"
																							action="multi_schema_backup" />
																					</menu:level>
																					<menu:logout />
																				</c:when>
																				<c:otherwise>
																			...
																			<c:choose>
																				...
																				<c:otherwise>
																					<menu:level module="search">
																						<menu:item module="search"
																							action="search_bibliographic" />
																						<menu:item module="search"
																							action="search_authorities" />
																						<menu:item module="search"
															...
															<div id=messages>
																...
																<% String schema=SchemaThreadLocal.get(); %>
																	...
																	<c:if test="${isLogged && !isDisableMenu}">
																		<c:set var="passwordWarning"
																			value='${schema}.system_warning_password' />
																		<c:set var="backupWarning"
																			value='${schema}.system_warning_backup' />
																		<c:set var="indexingWarning"
																			value='${schema}.system_warning_reindex' />
																		<c:set var="updateWarning"
																			value='${schema}.system_warning_new_version' />

																		<c:if test='${sessionScope[passwordWarning]}'>
																			<div
																				class="message sticky error system_warning_password">
																				<div>
																					<i18n:text
																						key="warning.change_password"
																						escapeHTML="true" />
																					<a href="?action=administration_password"
																						class="fright">
																						<i18n:text key="warning.fix_now"
																							escapeHTML="true" />
																					</a>
																				</div>
																			</div>
																		...
																		<c:if test="${sessionScope[backupWarning]}">
																			<div
																				class="message sticky error system_warning_backup">
																				<div>
																					<i18n:text
																						key="warning.create_backup"
																						escapeHTML="true" />
																					<a href="?action=administration_maintenance"
																						class="fright">
																						<i18n:text key="warning.fix_now"
																							escapeHTML="true" />
																					</a>
																		...
																		<c:if test="${sessionScope[indexingWarning]}">
																			<div
																				class="message sticky error system_warning_reindex">
																				<div>
																					<i18n:text
																						key="warning.reindex_database"
																						escapeHTML="true" />
																					<a href="?action=administration_maintenance"
																						class="fright">
																						<i18n:text key="warning.fix_now"
																		...
																		<c:if test="${sessionScope[updateWarning]}">
																			<div
																				class="message sticky error system_warning_new_version">
																				<div>
																					<div class="fright">
																						<a href="javascript:void(0)"
																							onclick="Core.ignoreUpdate(this);"
																							class="close"
