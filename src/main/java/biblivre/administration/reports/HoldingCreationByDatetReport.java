...
public class HoldingCreationByDatetReport extends BaseBiblivreReport {
    ...
    protected void generateReportBody(Document document, BaseReportDto reportData)
        ...
        String dateSpan =
                this.getText("administration.reports.field.date_from")
                        + " "
                        + dto.getInitialDate()
                        + " "
                        + this.getText("administration.reports.field.date_to")
                        + " "
                        + dto.getFinalDate();
        ...
        if (userTotal.size() > 0) {
            ...
            for (String name : userTotal.keySet()) {
                ...
                cell =
                        new PdfPCell(
                                new Paragraph(
                                        this.getNormalChunk(String.valueOf(userTotal.get(name)))));
                cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                table.addCell(cell);
            }
        ...
        Paragraph p3 =
                new Paragraph(
                        this.getHeaderChunk(
                                this.getText("administration.reports.field.database_count")));
        p3.setAlignment(Element.ALIGN_CENTER);
        document.add(p3);
        document.add(new Phrase("\n"));
