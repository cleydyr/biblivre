...
@Component
public class AllUsersAllDataReport extends BaseBiblivreReport {
    ...
    protected void generateReportBody(Document document, BaseReportDto reportData)
        ...
        Paragraph p1 = new Paragraph(getText("administration.reports.title.all_users"));
        p1.setAlignment(Element.ALIGN_CENTER);
        document.add(p1);
        document.add(new Phrase("\n"));
        Paragraph p2 =
                new Paragraph(
                        getHeaderChunk(getText("administration.reports.field.user_count_by_type")));
        ...
        if (listTable != null) {
            Paragraph p3 =
                    new Paragraph(
                            getHeaderChunk(
                                    getText("administration.reports.field.user_list_by_type")));
    ...
    private final PdfPTable createSummaryTable(Map<String, Integer> tipos) {
        ...
        for (Entry<String, Integer> entry : tipos.entrySet()) {
            ...
            total += count;
            cell = new PdfPCell(new Paragraph(getHeaderChunk(entry.getKey().toUpperCase())));
            cell.setBackgroundColor(HEADER_BG_COLOR);
            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
            cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
            cell.setColspan(2);
            table.addCell(cell);
            cell = new PdfPCell(new Paragraph(getNormalChunk(String.valueOf(count))));
    ...
    private final ArrayList<PdfPTable> createListTable(Map<String, List<String>> data) {
        try {
            ...
            for (Entry<String, List<String>> entry : data.entrySet()) {
                ...
                cell = new PdfPCell(new Paragraph(getHeaderChunk(description.toUpperCase())));
                cell.setColspan(4);
                ...
                for (String line : entry.getValue()) {
                    ...
                    // Nome
                    cell = new PdfPCell(new Paragraph(getNormalChunk(dados[0])));
                    cell.setHorizontalAlignment(Element.ALIGN_LEFT);
                    cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                    table.addCell(cell);

                    // Matricula
                    cell = new PdfPCell(new Paragraph(getNormalChunk(dados[1])));
                    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                    cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                    table.addCell(cell);

                    // Data de Inclusao
                    cell = new PdfPCell(new Paragraph(getNormalChunk(dados[2])));
                    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                    cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                    table.addCell(cell);

                    // Data de Cancelamento/Alteracao
                    cell = new PdfPCell(new Paragraph(getNormalChunk(dados[3])));
                    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                    cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
                    table.addCell(cell);
                }
