...
public class DataMigrationDAO extends AbstractDAO {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (recordType == RecordType.BIBLIO) {
                    ...
                    dto.setRecordDatabase(
                            (database == 0) ? RecordDatabase.MAIN : RecordDatabase.WORK);
    ...
    public List<HoldingDTO> listCatalogingHoldings(int limit, int offset) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                dto.setRecordDatabase(
                        (recordDatabase == 0) ? RecordDatabase.MAIN : RecordDatabase.WORK);

                Integer availability = rs.getInt("availability");
                dto.setAvailability(
                        (availability == 0)
                                ? HoldingAvailability.AVAILABLE
                                : HoldingAvailability.UNAVAILABLE);
    ...
    public List<MemoryFile> listDigitalMedia(int limit, int offset) {
        List<MemoryFile> list = new ArrayList<MemoryFile>();
    ...
    public List<AccessCardDTO> listAccessCards(int limit, int offset) {
        List<AccessCardDTO> list = new ArrayList<AccessCardDTO>();
    ...
    public List<Z3950AddressDTO> listZ3950Servers(int limit, int offset) {
        List<Z3950AddressDTO> list = new ArrayList<Z3950AddressDTO>();
    ...
    public List<AccessControlDTO> listAccessControl(int limit, int offset) {
        List<AccessControlDTO> list = new ArrayList<AccessControlDTO>();
    ...
    public List<LendingDTO> listLendings(int limit, int offset) {
        ...
        try {
            ...
            StringBuilder sql = new StringBuilder();
            sql.append(
                    "SELECT * FROM (SELECT lending_serial as id, holding_serial, user_serial, lending_date, return_date as expected_return_date, null as return_date  FROM lending ");
            sql.append("UNION  ");
            sql.append(
                    "SELECT lending_history_serial as id, holding_serial, user_serial, lending_date, null as expected_return_date, return_date as return_date FROM lending_history ) A ");
