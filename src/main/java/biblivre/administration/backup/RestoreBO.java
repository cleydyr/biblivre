...
public class RestoreBO extends AbstractBO {
    ...
    public boolean restore(RestoreDTO dto, RestoreDTO partial) {
        ...
        try {
            ...
            throw new ValidationException(
                    "administration.maintenance.backup.error.couldnt_restore_backup", e);
        } finally {
        ...
        try {
            ...
            try {
                DataMigrationDAO dao =
                        DataMigrationDAO.getInstance(this.getSchema(), "biblivre4_b3b_restore");
    ...
    public synchronized boolean restoreBackup(RestoreDTO dto, File path) {
        ...
        for (String originalSchemaName : restoreSchemas.keySet()) {
            // Verificamos o nome de destino do schema. Lembrando que o usuário pode escolher para
            // qual schema o backup será restaurado.
            // Isso significa que o originalSchemaName (nome do schema no backup) pode ser diferente
            // do finalSchemaName (nome do schem depois de restaurado).
        ...
        for (String originalSchemaName : restoreSchemas.keySet()) {
            // Verifica se o schema está atrapalhando alguma importação. Neste caso, renomeia
            // temporariamente.
            // Se o schema estiver na lista de schemas a serem deletados, não se preocupar em
            // renomear e restaurar
        ...
        try {
            ...
            if (restoreSchemas.containsKey(Constants.GLOBAL_SCHEMA)) {
                ...
                processRestore(
                        new File(path, Constants.GLOBAL_SCHEMA + ".schema." + extension), bw);
            ...
            for (String schema : restoreSchemas.keySet()) {
                ...
                this.processMediaRestore(
                        new File(path, schema + ".media." + extension), bw, schema);
            ...
            for (String renamedSchemaName : postRenameSchemas.keySet()) {
                ...
                State.writeLog(
                        "Renaming schema " + renamedSchemaName + " to " + originalSchemaName);
                bw.write(
                        "ALTER SCHEMA \""
                                + renamedSchemaName
                                + "\" RENAME TO \""
    ...
    public synchronized boolean recreateBiblivre3RestoreDatabase(boolean tryPGSQL92) {
        ...
        String[] commands =
                new String[] {
                    psql.getAbsolutePath(), // 0
                    "--host", // 1
                    DatabaseUtils.getDatabaseHostName(), // 2
                    "--port", // 3
                    DatabaseUtils.getDatabasePort(), // 4
                    "-v", // 6
                    "ON_ERROR_STOP=1", // 7
                    "--file", // 8
                    "-", // 9
    ...
    public static void processRestore(File restore, BufferedWriter bw) throws IOException {
        ...
        BufferedReader reader =
                new BufferedReader(new InputStreamReader(new FileInputStream(restore), "UTF-8"));
        ...
        while (sc.hasNextLine()) {
            ...
            if (inputLine.startsWith("SELECT pg_catalog.lo_create")) {
                ...
                if (loOpenMatcher.find()) {
                    String newLine =
                            loOpenMatcher.replaceFirst(
                                    "$1" + oidMap.get(loOpenMatcher.group(2)) + "$3");
