...
public class Update implements UpdateService {
    ...
    private void _fixBackupTable(Connection connection) throws SQLException {
        String createGlobalBackupTableSQL =
                "CREATE TABLE global.backups (id serial NOT NULL, "
    ...
    private static void _fixUpdateUserFunction(Connection connection) throws SQLException {
        String sql =
                "CREATE OR REPLACE FUNCTION update_user_value(integer, character varying, character varying, character varying) RETURNS integer \n"
                        + "  LANGUAGE plpgsql \n"
                        + "    AS $_$ \n"
                        + " DECLARE \n"
                        + "	p_id ALIAS FOR $1; \n"
    ...
    private static void _fixUpdateTranslationFunction(Connection con) throws SQLException {
        String sql =
                "CREATE OR REPLACE FUNCTION update_translation(character varying, character varying, character varying, integer) RETURNS integer \n"
                        + "    LANGUAGE plpgsql \n"
                        + "    AS $_$ \n"
                        + " DECLARE \n"
