...
export async function navigateToCirculationLending(page: Page): Promise<void> {
  const circulationNav = page.getByRole("button", { name: "Circulação" });
  ...
  const lendingNav = page.getByRole("button", {
    name: "Empréstimo e renovação",
  });

  await expect(lendingNav).toBeEnabled();
  await lendingNav.click();

  await expect(page).toHaveURL(/\/spa\/circulation_lending/);
  await expect(
